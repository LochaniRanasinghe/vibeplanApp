...
class AdminDashboardController extends Controller
{
    public function index()
    {
        ...
        $eventRevenue = EventInventoryOrder::with(['customEvent.eventRequest.eventType', 'inventoryItem'])
            ->where('status', 'approved')
            ->get()
            ->groupBy(function ($order) {
                return optional($order->customEvent->eventRequest->eventType)->name ?? 'Unknown Type';
            })
            ->map(function ($orders) {
                $totalRevenue = $orders->sum(function ($order) {
                    return optional($order->inventoryItem)->price_per_unit * $order->quantity;
                });
                $itemList = $orders->pluck('inventoryItem.item_name')->unique()->filter()->values();
        ...
        return view('admin.dashboard', compact('salesPerItem', '$eventRevenue = EventInventoryOrder::with(['customEvent.eventRequest.eventType', 'inventoryItem'])
    ->where('status', 'approved')
    ->get()
    ->groupBy(function ($order) {
        return optional($order->customEvent->eventRequest->eventType)->name ?? 'Unknown Type';
...
', 'eventRequests', 'monthlyItemRevenue', 'salesOverTime', 'salesByEvent'));
    }
}
